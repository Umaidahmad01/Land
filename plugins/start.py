...
from bot import Bot
from config import FORCE_MSG, OWNER_ID, START_MSG, CUSTOM_CAPTION, DISABLE_CHANNEL_BUTTON, PROTECT_CONTENT, AUTO_DEL, DEL_TIMER, DEL_MSG, START_PIC, FORCE_PIC, DEV_ID
...
# Handle start command for not joined users
...
async def not_joined(client: Client, message: Message):
    buttons = [
        ...
        [
            InlineKeyboardButton(text="• ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ •", url='https://t.me/+eTdO3poYlbwyNDY9'),
        ]
    ]
    try:
        buttons.append(
...
# Command to get number of users using the bot
@Bot.on_message(filters.command('users') & filters.private)
async def get_users(client: Bot, message: Message):
    user_id = message.from_user.id
    is_admin = await present_admin(user_id)
    
    if not is_admin & user_id != DEV_ID & OWNER_ID:
